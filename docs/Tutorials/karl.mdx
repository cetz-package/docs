...
There are ifferent ways of specifying coordinates. The easiest way is to say something like `(10pt, 2cm)`. This means 10pt in $x$-direction and 2cm in $y$-directions. Alternatively, you can also leave out the units as in `(1, 2)`, which means "one times the unit length in the $x$-direction and twice the unit length in the $y$-direction". The unit length defaults to 1cm in both directions.
